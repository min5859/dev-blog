...
const logPath = path.join(logDir, `${runDate}-ai-coding-agents.log`);
const latestLogPath = path.join(logDir, 'ai-coding-agents-latest.log');
const statusPath = path.join(logDir, 'ai-coding-agents-latest-status.json');
...
function renderLog(status) {
  const lines = [`# AI 코딩 에이전트 파이프라인 ${status.runDate}`, `startedAt: ${status.startedAt}`, `finishedAt: ${status.finishedAt}`, `ok: ${status.ok}`, ''];
...
async function main() {
  ...
  const status = { topic: 'ai-coding-agents', runDate, startedAt, finishedAt: new Date().toISOString(), ok: results.every((r) => r.ok) && results.length === steps.length, steps: results, publishEnabled: shouldPublish };
  await writeFile(logPath, renderLog(status));
  await writeFile(latestLogPath, renderLog(status));
  await writeFile(statusPath, JSON.stringify(status, null, 2));
  console.log(`AI 코딩 에이전트 파이프라인 ${status.ok ? 'succeeded' : 'failed'}; log: ${path.relative(root, logPath)}`);
  if (!status.ok) process.exit(1);
}

main().catch(async (error) => {
  await mkdir(logDir, { recursive: true });
  await writeFile(statusPath, JSON.stringify({ topic: 'ai-coding-agents', runDate, startedAt, finishedAt: new Date().toISOString(), ok: false, error: error.stack || error.message, steps: [] }, null, 2));
  console.error(error);
  process.exit(1);
});
