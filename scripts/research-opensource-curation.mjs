...
async function main() {
  ...
  const { dossier, out } = await runResearch({
    ...
    promptTemplatePath: path.join(root, 'prompts', 'opensource-curation-research-ko.md'),
    adapter,
    generatedAt: new Date().toISOString(),
    defaultAudience: '오픈소스 도입·평가를 검토하는 개발자',
  });
  console.log(`Built opensource-curation research dossier with ${adapter} adapter; ${dossier.entries.length} entr(ies), ${dossier.droppedCandidates?.length || 0} dropped; wrote ${path.relative(root, out)}`);
}
