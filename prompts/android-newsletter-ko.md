...
`title` 은 `{date} {핵심사건} — {기존 토픽 이름}` 으로 작성합니다.
- `{date}` 는 입력 draft 의 `date` 그대로.
- `{핵심사건}` 은 본문이 가장 중요하게 다루는 한 가지 사건의 **12자 안팎** 한국어 요약. 약어·해시·식별자 금지.
- `{기존 토픽 이름}` 은 토픽 라벨 (`Android 커널`).
- 좋은 예: `2026-05-12 ashmem 메모리 안전 버그 5건 — Android 커널`
- 나쁜 예: `2026-05-12 Android 커널 데일리 브리핑` (사건 없음)
...
- 좋은 예: `Android 커널에 ashmem 메모리 안전 버그 5건이 한 묶음으로 고쳐졌습니다. ACK 빌드팀은 백포트 확인 권장.`
...
세 필드 모두 행동 지침 어미("…확인하세요" 등) 로 끝맺습니다. 위에 적힌 hallucination 가드(식별자·플래그·URL 발명 금지) 는 그대로 적용됩니다.
...
"highlights": [
  {
    ...
    "verify": "ABI·동작 테스트를 ASHMEM_GET_NAME 호출 경로에 돌려 확인하세요"
  }
]
```
