...
- `do` — *무엇을 하면 되는가.* 한 문장, 50자 안팎. **"확인하세요"·"점검하세요"로만 끝내는 것은 금지.** 동사는 구체적인 행동(대조하세요 / grep하세요 / cherry-pick하세요 / diff로 비교하세요 / 패치를 적용하세요 / 트리에 업데이트하세요)을 써야 합니다. 막연한 "확인하세요" 대신 무엇을, 어떻게 하는지 동사에 담으세요.
- `verify` — *어디서 어떻게 검증하는가.* 한 문장, 60자 안팎.

위에 적힌 가드는 그대로 적용됩니다.

**do 필드 NG vs OK 예시:**
- NG: `"do": "변경된 인터페이스를 확인하세요"` ← 무엇을 어떻게 해야 하는지 불분명
- OK: `"do": "v10 패치와 자기 트리의 KCFI 초기화 순서를 diff로 비교하세요"`
- NG: `"do": "영향 받는 경로를 점검하세요"` ← 막연
- OK: `"do": "irqfd 경로를 grep해 cleanup 호출 순서가 일치하는지 대조하세요"`
...
"highlights": [
  {
    ...
    "do": "v10 패치와 자기 트리의 KCFI 초기화 순서를 diff로 비교하세요",
