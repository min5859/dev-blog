...
- **`candidateBodies`를 반드시 활용합니다.** 각 LKML 후보의 실제 commit message가 들어 있습니다. 제목을 한국어로 *풀어쓰는 것*이 아니라, commit message를 읽고 *무엇이 구체적으로 바뀌고 어떤 코드 경로/계약이 달라지는지* 한 문장으로 답합니다. commit message가 비어 있을 때만 메타데이터 기반 보수 표현으로 폴백합니다.
