...
- 각 항목: `title`(owner/repo), `priority` (`상`/`중`/`하`), `impactType`(항상 `project`), `affectedAudience`(도입 검토자 그룹), `verifyLink`, `action`(다음에 할 일 한 줄).
...
```json
{
  ...
  "highlights": [
    { "title": "owner/repo", "priority": "상", "impactType": "project", "affectedAudience": "Rust 도입 검토자", "verifyLink": "https://github.com/...", "action": "..." }
  ],
  "sections": [
